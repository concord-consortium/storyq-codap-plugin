...
20. Extracting single words assigns one color, pale yellow `#fffacd`, to the ngram feature and to every token it produces, instead of cycling the six colors per word. The color is deliberately **not** one of the six, so that an ordinary feature and the extracted words can never render identically.
21. The ngram feature row in the Features tab shows that color rather than rendering colorless as it does today.
22. **Conditional on the ngram feature carrying `kNoColor`**, restoring a document sets that feature's color to `#fffacd` and then sets every **unigram** entry in the restored `tokenMap` to **the feature's repaired color**. If the ngram feature already has a real color, the whole step is skipped, tokens included.
...
- ~~**The yellow collision**~~ — resolved on 2026-08-08, see Decisions. Keeping the two yellows as independent constants rather than aliasing them is what made the fix a one-line edit.
- **`#fffacd` is faint, and that was the accepted trade in choosing it.** It sits 15.5 from white in CIEDE2000, against 19.7 to 31.3 for the six, so it is the palest thing the plugin renders as a highlight. This is inherent to the choice rather than a defect: within yellow, the only way to get far enough from `#ffe671` is to fade toward white, and Jie picked the color knowing the shortlist said so. Worth watching in classroom use, particularly on a projector or a washed-out laptop screen, where some of the 682 extracted words may not read as highlighted at all. If that turns out to bite, the fix is the same one-line edit, and the measured alternatives are in Decisions.
...
**Decision**: **A**, answered by Jie Chao 2026-08-07: "I agree that color for single words should be different from the six colors. I prefer to find another color outside of the six, maybe a lighter or slightly different yellow. If it is not easy to find that color, you can also use the current yellow for single words and leave the other five for hand-picked words." B was her own fallback and was not needed. **A costs one thing B would not have**: a color outside the six is not offered by the picker, so choosing away from it would have stranded the default. Requirement 15a buys it back with a seventh swatch on that row only.

**Which color, specifically**: `#fffacd`, chosen by Jie from a measured shortlist on 2026-08-08. Contrast against `#222222` is 15.01:1, up from the palette yellow's 12.73:1, so requirement 39's guarantee is unaffected.

The shortlist was measured rather than eyeballed, because the first candidate offered, `#fff3b0`, failed a test nobody had thought to apply. **The right metric is CIEDE2000 perceptual distance, not WCAG contrast**, which measures text against its background and says nothing about whether two highlights look alike. And there is a natural bar for it: the closest pair among the six, Purple and Pink, sit 11.7 apart, and the design already treats those two as telling apart, so any single-words color scoring below 11.7 asks students for a finer distinction than the palette itself ever asks of them. `#fff3b0` scored 8.8. So did four of the six yellows on the shortlist; `#ffef8f` scored 4.1, and `#ffd84d` scored 4.9 despite looking decisively different, because saturation separates two colors far less reliably than hue does.

Non-yellows were offered alongside, since the six occupy 49, 79, 178, 198, 272 and 311 degrees of the hue wheel and leave 98 degrees through red and orange unused, which is where the distance is. `#ffbfa3` scored 26.0, more than twice the chosen color. Jie preferred to stay with yellow. `#fffacd` scores 13.9, which clears the bar, and it is the choice that keeps her original "lighter yellow" instinct intact. Its cost is recorded under Not Yet Implemented: it is the second faintest candidate measured against white, so it leans on the pale end of "still reads as a highlight".

Two further colors were measured and set aside before Jie saw the list: `#ffc2c2` scores well but red carries meaning in a tool whose classes are named positive and negative, and the neutrals `#ece7dc` and `#e4e6e8` sit 7.2 and 5.4 from white, which collides with requirement 11's white-pill meaning of "highlighting off".
