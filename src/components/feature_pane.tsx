...
import { getFeatureColor, kNoColor, ngramColor } from "../utilities/color-utils";
  ...
  const handleClick = action(async () => {
    if( featureUnderConstruction.inProgress) {
      if(featureUnderConstruction.info.kind === kFeatureKindNgram && featureStore.hasNgram) {
        ...
        if (featureUnderConstruction.color === kNoColor) {
          featureUnderConstruction.color = featureUnderConstruction.info.kind === kFeatureKindNgram
            ? ngramColor
            : getFeatureColor();
        }
